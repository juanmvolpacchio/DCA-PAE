...
export default function WaterChart({ series, points, addNewPoint, savedCurve, showNewCurve, editableParams }) {
  ...
  const savedCurveData = savedCurve && savedCurve.start_date
    ? (() => {
        ...
        const savedCurveExtrapolated = calculateExtrapolatedWater(
  ...
  const newCurveData = showNewCurve && editableParams?.["Seg. 1"]
    ? (() => {
        ...
        const totalMonths = series.months.length - startIndex + extrapolationMonths;
        const newCurveExtrapolated = calculateExtrapolatedWater(
